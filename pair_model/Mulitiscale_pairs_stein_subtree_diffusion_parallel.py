...
ind_inits = [450]
# funcs = ['linear','log','sigmoid','random']
funcs = ['sigmoid'] #,'linear','random']

work = [[a,b] for b in ind_inits for a in funcs]
...
def diffusion(p):
    ...
    for j, d, col, per, s in z:
        ...
        for i in range(no_sims):
            print("Sim: " + str(i))
            while(True):
                model = InfectionMultiscaleModel(pop, steps_per_day=steps_per_day, v_threshold=10000, G=p[0], con_init=p[1], sig_init=s, delay=d, lin_init=0.01, log_init=400)
        ...
        ptr.savefig(fname='trans_tree_' + str(current_process()))
        # ptr.show()
        ...
        binned_gen = []
        bins_gen = np.linspace(0,25,num=26)
        for g in bins_gen:
            binned_gen.append([])
        ...
        ptr.savefig(fname='subtree_diffusion_' + str(p[0]) + '_' + str(list.index(ind_inits,p[1])))
        # ptr.show()
        ...
        gen_data = pd.DataFrame(data = binned_gen[1:]) #, columns=labs_gen)

        print(gen_data)
        # print(sns.load_dataset('iris'))

        sns.stripplot(data=gen_data, ax=axes_a, jitter=0.2)
        sns.despine()

        # sns.stripplot(x=labs_time, y=binned_time[1:], ax=axes_b, jitter=0.2)
        # sns.despine()

        ptr.tight_layout()

        ptr.savefig(fname='subtree_diffusion_noCIs_' + str(p[0]) + '_' + str(list.index(ind_inits,p[1])))
        # ptr.show()
